...
const ProductInfoTabs = ({ 
  ...
  React.useEffect(() => {
    if (activeTab === 'inventory') {
      console.log('Inventory Tab Debug:');
      ...
      if (productDetails?.data) {
        ...
        if (productDetails.data?.data) {
          console.log('Actual Data:', productDetails.data.data);
          console.log('Actual Inventory:', productDetails.data.data.inventory);
          console.log('Actual Basic:', productDetails.data.data.basic);
          
          // Log all inventory fields
          const inventory = productDetails.data.data.inventory || {};
      ...
      console.log('Inventory Stock:', productDetails?.inventory?.stockQuantity);
      console.log('Track Quantity:', productDetails?.inventory?.trackQuantity);
      console.log('Product Track Quantity:', product?.trackQuantity);
    }
  ...
  const renderOverviewTab = () => {
    // Extract data from the correct location in the API response
    const actualData = productDetails?.data?.data || {};
    const mediaData = actualData?.media || {};
    const pricingData = actualData?.pricing || {};
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <ProductImageGallery
              images={mediaData?.images || [product?.imageUrl]}
                ...
                {pricingData?.comparePrice && (
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-500 line-through">
                      {formatCurrency(pricingData.comparePrice)}
                    </p>
                  </div>
                )}
              </div>
  ...
  const renderAnalyticsTab = () => {
    // Extract data from the correct location in the API response
    const actualData = productDetails?.data?.data || {};
    const analyticsData = actualData?.analytics || {};
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-green-900">
                  {formatNumber(analyticsData?.totalSales || 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <p className="text-3xl font-bold text-gray-900">
                {((analyticsData?.addToCartRate || 0) * 100).toFixed(1)}%
              </p>
            ...
            <div className="text-center">
              <p className="text-3xl font-bold text-gray-900">
                {formatNumber(analyticsData?.reviewCount || 0)}
              </p>
  ...
  const renderInventoryTab = () => {
    // Extract inventory data from the correct location in the API response
    const actualData = productDetails?.data?.data || {};
    const inventoryData = actualData?.inventory || {};
    const basicData = actualData?.basic || {};
    
    // Access the correct field names based on the API response
    const stockQuantity = product?.stockQuantity || inventoryData?.stockQuantity || 0;
    const trackQuantity = inventoryData?.trackQuantity === true;
    const lowStockThreshold = inventoryData?.minStockAlert || 10;
    
    console.log('Using stockQuantity:', stockQuantity);
    console.log('Using trackQuantity:', trackQuantity);
    console.log('Using lowStockThreshold:', lowStockThreshold);
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatNumber(lowStockThreshold)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <p className="text-sm text-gray-500">SKU: {product?.sku || basicData?.sku || 'N/A'}</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-gray-900">
                  {formatNumber(stockQuantity)} units
                </p>
                <p className={`text-sm ${
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          ...
          <p className="text-blue-700 mb-4">
            Based on sales velocity, we recommend reordering when stock drops below {
              Math.ceil((inventoryData?.minStockAlert || 10) * 1.5)
            } units.
          </p>
  ...
  const renderActionsTab = () => {
    ...
    // Access the correct field names based on the API response
    const trackQuantity = inventoryData?.trackQuantity === true;
    const allowBackorders = inventoryData?.allowBackorders === true;
    const stockQuantity = inventoryData?.stockQuantity || 0;
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900">{formatCurrency(product?.price || basicData?.price)}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Stock</label>
                    <p className="text-lg font-bold text-gray-900">{stockQuantity} units</p>
                  </div>
                </div>
                <div>
                  <label className="text-sm text-gray-500">Status</label>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center">
              <input
                type="checkbox"
                className="h-4 w-4 text-green-600 rounded border-gray-300"
                defaultChecked={allowBackorders}
              />
